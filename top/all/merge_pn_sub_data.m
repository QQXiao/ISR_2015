...
ln_mmem=[];mem_mmem=[];
	...
	ln_lnfile=sprintf('%s/ln_ln_sub%02d.mat',psdir,s);           
	mem_lnfile=sprintf('%s/mem_ln_sub%02d.mat',psdir,s);           
	mem_memfile=sprintf('%s/mem_mem_sub%02d.mat',psdir,s);           
	ln_memfile=sprintf('%s/ln_mem_sub%02d.mat',psdir,s);           
...
eval(sprintf('save %s/allsub_ln_mln.txt ln_mln_all -ascii -tabs', psdir));
eval(sprintf('save %s/allsub_mem_mln.txt mem_mln_all -ascii -tabs', psdir));
eval(sprintf('save %s/allsub_ln_mmem.txt ln_mmem_all -ascii -tabs', psdir));
eval(sprintf('save %s/allsub_mem_mmem.txt mem_mmem_all -ascii -tabs', psdir));
end %function
