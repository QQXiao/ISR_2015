...
datadir=sprintf('%s/top/tmap/data/p_based',basedir);
resultdir=sprintf('%s/top/tmap/ps/p_based',basedir);
d=1/96;
plist_all=0:d:1;
...
for s=subs
	for hd=1:2
		...
		nn=plist(n);
		ln_file=sprintf('%s/p%d_ERSI_sub%02d_h%d.mat', datadir,n,s,hd);
		mem_file=sprintf('%s/p%d_ERSD_sub%02d_h%d.mat', datadir,n,s,hd);
		end
		load(ln_file);
