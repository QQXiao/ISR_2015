...
datadir=sprintf('%s/top/tmap/data/p_based/new_vvc_sep_roi',basedir);
resultdir=sprintf('%s/top/tmap/ps/p_based/new_vvc_sep_roi',basedir);
%%%%%%%%%
d=1/96;
plist_all=0:d:1;
...
for s=subs
	...
	for hd=1:2
		...
		nn=plist(n);
		ln_file=sprintf('%s/%s_p%d_ln_sub%02d_h%d.mat',datadir,roi_name{roi},n,s,hd);
		mem_file=sprintf('%s/%s_p%d_mem_sub%02d_h%d.mat',datadir,roi_name{roi},n,s,hd);
