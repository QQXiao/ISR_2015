...
infodir=sprintf('%s/top/tmap/data/p_based/new_vvc_sep_roi',basedir);
...
plist=plist_need([1:5:46 47:end])';
...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	pln1=mean(tpln1,2);
	pln2=mean(tpln2,2);
        for pn=1:length(plist)
		for h=1:2
		eval(sprintf('pln=pln%d;',h));
		vln=find(pln>=plist(pn));
        	data_vln=ttvvc(:,vln);
        	file_name=sprintf('%s/%s_p%d_ln_sub%02d_h%d.mat',infodir,roi_name{roi},pn,s,h);
	...
	for pn=1:length(plist)
        	...
        	vmem=find(ppmem>=plist(pn));
        	data_vmem=ttvvc(:,vmem);          
        	file_name=sprintf('%s/%s_p%d_mem_sub%02d_h%d.mat',infodir,roi_name{roi},pn,s,h);
