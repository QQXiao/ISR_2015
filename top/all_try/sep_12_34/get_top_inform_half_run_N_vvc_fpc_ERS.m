...
infodir=sprintf('%s/top/tmap/data/value_based/VVC_FPC',basedir);
...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	ttvvc_all=[tmp_VVC(:,1:end) tmp_IPL(:,1:end) tmp_IFG(:,1:end) tmp_MFG(:,1:end)];
	...
	pa=combntns([1:ss(1)],2);
       ...
       for pn=[0:5:95 96:1:99]
                ...
                file_name=sprintf('%s/p%d_ERSI_sub%02d_h%d',infodir,pn,s,h);          
                eval(sprintf('save %s data_vERS',file_name));                         
                file_name=sprintf('%s/p%d_ERSI_sub%02d_withc_h%d',infodir,pn,s,h);    
        ...
        for pn=[0:5:95 96:1:99]                                                       
                ...
                data_vERS=ttvvc(:,vERS);                                              
                data_cvERS=ttvvc_all(:,vERS);                                         
                file_name=sprintf('%s/p%d_ERSD_sub%02d_h%d',infodir,pn,s,h);          
                eval(sprintf('save %s data_vERS',file_name));                         
                file_name=sprintf('%s/p%d_ERSD_sub%02d_withc_h%d',infodir,pn,s,h);    
                eval(sprintf('save %s data_cvERS',file_name));                        
                end %half                                                             
        end %pn                                                                       
end%sub                                                                               
