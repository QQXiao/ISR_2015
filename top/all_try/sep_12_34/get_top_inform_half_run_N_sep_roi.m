...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	pln2=mean(tpln2,2);
		...
		tpln=unique(pln);
        	ln_pn=prctile(tpln,pn);
        	vln=find(pln>=ln_pn);
        	data_vln=ttvvc(:,vln);
        	data_cvln=ttvvc_all(:,vln);
        	file_name=sprintf('%s/%s_p%d_ln_sub%02d_h%d',infodir,roi_name{roi},pn,s,h);
        	eval(sprintf('save %s data_vln',file_name));
        	file_name=sprintf('%s/%s_p%d_ln_sub%02d_withc_h%d',infodir,roi_name{roi},pn,s,h);
	...
	for pn=[0:5:95 96:1:99]
		...
		tppmem=unique(ppmem);
        	mem_pn=prctile(tppmem,pn);
        	vmem=find(ppmem>=mem_pn);
        	data_vmem=ttvvc(:,vmem);          
        	data_cvmem=ttvvc_all(:,vmem);          
        	file_name=sprintf('%s/%s_p%d_mem_sub%02d_h%d',infodir,roi_name{roi},pn,s,h);
        	eval(sprintf('save %s data_vmem',file_name));
        	file_name=sprintf('%s/%s_p%d_mem_sub%02d_withc_h%d',infodir,roi_name{roi},pn,s,h);
