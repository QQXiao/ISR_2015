...
resultdir=sprintf('%s/top/tmap/ps/number_based_WB',basedir);
%%%%%%%%%
for s=subs
	for hd=1:2
		for hr=1:2	
		...
		nn=tn(n);
		if nn <= 99
		ln_file=sprintf('%s/p%d_ERSI_sub%02d_h%d.mat', datadir,nn,s,hd);
		mem_file=sprintf('%s/p%d_ERSD_sub%02d_h%d.mat', datadir,nn,s,hd);
		else
		ln_file=sprintf('%s/p%.1f_ERSI_sub%02d_h%d.mat', datadir,nn,s,hd);
                mem_file=sprintf('%s/p%.1f_ERSD_sub%02d_h%d.mat', datadir,nn,s,hd);
		end
		load(ln_file);
