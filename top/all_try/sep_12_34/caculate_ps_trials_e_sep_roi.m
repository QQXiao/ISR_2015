...
datadir=sprintf('%s/top/tmap/data/equal/sep_roi/trials',basedir);
resultdir=sprintf('%s/top/tmap/ps/equal/sep_roi/trials',basedir);
%%%%%%%%%
...
for s=subs
	for roi=1:length(roi_name);
		for hd=1:2
			...
			for n=1:length(tn)
			nn=tn(n);
			ln_file=sprintf('%s/p%d_ln_sub%02d_h%d_%s', datadir,nn,s,hd,roi_name{roi});
			mem_file=sprintf('%s/p%d_mem_sub%02d_h%d_%s', datadir,nn,s,hd,roi_name{roi});
