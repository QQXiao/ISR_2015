...
resultdir=sprintf('%s/top/tmap/ps/number_based/sep_roi',basedir);
%%%%%%%%%
roi_name={'VVC','FPC'};
for s=subs
	for roi=1:2
	for hd=1:2
		...
		nn=tn(n);
		if nn <= 99
                ln_file=sprintf('%s/%s_p%d_ERSI_sub%02d_h%d.mat', datadir,roi_name{roi},nn,s,hd);
                mem_file=sprintf('%s/%s_p%d_ERSD_sub%02d_h%d.mat', datadir,roi_name{roi},nn,s,hd);
                else
