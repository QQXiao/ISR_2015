...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	pln2=mean(tpln2,2);
                for h=1:2                                                                                      
                eval(sprintf('ppln=pln%d',h));                                                               
                tppln=unique(ppln);                                                                          
                for nnln=1:length(tppln)
                vln=find(ppln==tppln(nnln));
                pvln(roi,h,nnln)=tppln(nnln);                                                                  
                nvln(roi,h,nnln)=length(vln);       
                end
                end %half  
	...
	if t<=144
		...
		tppmem=unique(ppmem);
		for nnmem=1:length(tppmem)
        	vmem=find(ppmem==tppmem(nnmem));
        	pvmem(roi,h,nnmem)=tppmem(nnmem);          
        	nvmem(roi,h,nnmem)=length(vmem);          
		end          
		end %half
	end %roi
end%sub
        	file_name=sprintf('%s/np_sub%02d',infodir,s);
        	eval(sprintf('save %s pvln nvln pvmem nvmem',file_name));
end %function
