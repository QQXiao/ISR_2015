...
infodir=sprintf('%s/top/tmap/data/number_based/sep_roi',basedir);
...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	[tidx_ERS_I,tidx_ERS_IB_all,tidx_ERS_IB_wc,tidx_ERS_D,tidx_ERS_DB_all,tidx_ERS_DB_wc,list_pid]=get_idx_item_ERS(s,t)
       ...
       for pn=[0:5:95 96:1:99 99.1 99.2 99.3 99.4 99.5]
		...
		if pn <=99
                file_name=sprintf('%s/%s_p%d_ERSI_sub%02d_h%d.mat',infodir,roi_name{roi},pn,s,h);          
                eval(sprintf('save %s data_vERS',file_name));                         
                file_name=sprintf('%s/%s_p%d_ERSI_sub%02d_withc_h%d.mat',infodir,roi_name{roi},pn,s,h);    
	...
	for v=1:ss(2)                                                         
        ...
        for pn=[0:5:95 96:1:99 99.1 99.2 99.3 99.4 99.5]                                                       
                ...
                data_cvERS=ttvvc_all(:,vERS); 
		if pn <= 99                                        
                file_name=sprintf('%s/%s_p%d_ERSD_sub%02d_h%d.mat',infodir,roi_name{roi},pn,s,h);          
                eval(sprintf('save %s data_vERS',file_name));                         
                file_name=sprintf('%s/%s_p%d_ERSD_sub%02d_withc_h%d.mat',infodir,roi_name{roi},pn,s,h);    
                eval(sprintf('save %s data_cvERS',file_name));                        
                else
		file_name=sprintf('%s/%s_p%.1f_ERSD_sub%02d_h%d.mat',infodir,roi_name{roi},pn,s,h);                             
                eval(sprintf('save %s data_vERS',file_name));                                                             
                file_name=sprintf('%s/%s_p%.1f_ERSD_sub%02d_withc_h%d.mat',infodir,roi_name{roi},pn,s,h);                       
                eval(sprintf('save %s data_cvERS',file_name));
		end
		end %half                                                             
        end %pn    
