...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	for roi=1:length(roi_name);
		...
		%%analysis
		    ...
		    cc_ln_set1(roi,:)=tcc_ln_set1;
		    cc_ln_set2(roi,:)=tcc_ln_set2;
		    cc_ln_mean(roi,:)=tcc_ln_mean;
		    cc_mem_set1(roi,:)=tcc_mem_set1;
		    cc_mem_set2(roi,:)=tcc_mem_set2;
		    cc_mem_mean(roi,:)=tcc_mem_mean;
        end %roi
cc_a_roi_ln_set1(s,:)=1-pdist(cc_ln_set1(:,:),'correlation');
cc_a_roi_ln_set2(s,:)=1-pdist(cc_ln_set2(:,:),'correlation');
cc_a_roi_ln_mean(s,:)=1-pdist(cc_ln_mean(:,:),'correlation');
cc_a_roi_mem_set1(s,:)=1-pdist(cc_mem_set1(:,:),'correlation');
cc_a_roi_mem_set2(s,:)=1-pdist(cc_mem_set2(:,:),'correlation');
cc_a_roi_mem_mean(s,:)=1-pdist(cc_mem_mean(:,:),'correlation');
end %end sub
cc_a_roi_ln_set1=cc_a_roi_ln_set1(subs,:,:);
cc_a_roi_ln_set2=cc_a_roi_ln_set2(subs,:,:);
cc_a_roi_ln_mean=cc_a_roi_ln_mean(subs,:,:);
cc_a_roi_mem_set1=cc_a_roi_mem_set1(subs,:,:);
cc_a_roi_mem_set2=cc_a_roi_mem_set2(subs,:,:);
cc_a_roi_mem_mean=cc_a_roi_mem_mean(subs,:,:);
c_ln_set1=squareform(squeeze(mean(cc_a_roi_ln_set1,1)));c_ln_set2=squareform(squeeze(mean(cc_a_roi_ln_set2,1)));c_ln_mean=squareform(squeeze(mean(cc_a_roi_ln_mean,1))); 
c_mem_set1=squareform(squeeze(mean(cc_a_roi_mem_set1,1)));c_mem_set2=squareform(squeeze(mean(cc_a_roi_mem_set2,1)));c_mem_mean=squareform(squeeze(mean(cc_a_roi_mem_mean,1)));   
eval(sprintf('save %s/all_corr_mem_set1.txt c_mem_set1 -ascii -tabs', resultdir));
eval(sprintf('save %s/all_corr_mem_set2.txt c_mem_set2 -ascii -tabs', resultdir));
eval(sprintf('save %s/all_corr_mem_mean.txt c_mem_mean -ascii -tabs', resultdir));
eval(sprintf('save %s/all_corr_ln_set1.txt c_ln_set1 -ascii -tabs', resultdir));
eval(sprintf('save %s/all_corr_ln_set2.txt c_ln_set2 -ascii -tabs', resultdir));
eval(sprintf('save %s/all_corr_ln_mean.txt c_ln_mean -ascii -tabs', resultdir));
end %function
