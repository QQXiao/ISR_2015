...
for s=subs
	...
	mem=zeros(size_all);
    	...
    	lnfile=sprintf('%s/p95_ln_sub%02d_withc.mat',datadir,s);
    	load(lnfile);
	...
	ln(c(1),c(2),c(3),:)=1;
    	...
    	memfile=sprintf('%s/p95_mem_sub%02d_withc.mat',datadir,s);
    	load(memfile);
	...
	smem=size(mask_mem);
        ...
        mem(c(1),c(2),c(3),:)=1;
       	...
       	filename=sprintf('%s/top95_ln_sub%02d.nii',resultdir,s);
       	...
       	filename=sprintf('%s/top95_mem_sub%02d.nii',resultdir,s);
