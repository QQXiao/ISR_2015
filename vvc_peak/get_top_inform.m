...
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	...
	data_ln_mean=(data_ln_set1+data_ln_set2)/2;
        ln_tcc_mean=1-pdist(data_ln_mean(:,:),'correlation');
	...
	data_mem_mean=(data_mem_set1+data_mem_set2)/2;
        mem_tcc_mean=1-pdist(data_mem_mean(:,:),'correlation');
end%sub
        ...
        file_name=sprintf('%s/p95_ln_sub%02d_mean', infodir,s);
        eval(sprintf('save %s ln_tcc_mean',file_name));
        file_name=sprintf('%s/p95_mem_sub%02d_mean', infodir,s);
        eval(sprintf('save %s mem_tcc_mean',file_name));
