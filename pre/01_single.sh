...
do
	...
	sed -e "s/ISR05/${SUB}/g" -e "s/encoding/${c}/g" -e "s/run1/run${r}/g" -e "s/set1/set${s}/g" $outputdir/script/design/design.fsf > $outputdir/script/fsf/single_${c}_${SUB}_run${r}_set${s}.fsf
        feat $outputdir/script/fsf/single_${c}_${SUB}_run${r}_set${s}.fsf
