function caulate_ps_matrix_p95(subs,n)
%%%%%%%%%
basedir='/seastor/helenhelen/ISR_2015';
addpath /seastor/helenhelen/scripts/NIFTI
addpath /home/helenhelen/DQ/project/gitrepo/ISR_2015/behav
datadir=sprintf('%s/top/z_after_model/data',basedir);
psdir=sprintf('%s/top/z_after_model/ps_matrix',basedir);
condname={'ln','mem'};
%%%%%%%%%
for s=subs
[idx_ERS_I,idx_ERS_IB_all,idx_ERS_IB_wc,idx_ERS_D,idx_ERS_DB_all,idx_ERS_DB_wc,idx_mem_D,idx_mem_DB_all,idx_mem_DB_wc,idx_ln_D,idx_ln_DB_all,idx_ln_DB_wc,m_ln,m_mem]= get_idx(s);
	for c=1:2
                xx=[];tmp_xx=[];
                load(sprintf('%s/p%d_%s_sub%02d.mat',datadir,n,condname{c},s));
		...
		cc_mem_mem(:)=tm(:);             		
        file_name=sprintf('%s/%s_top%d_mps_ln_sub%02d',psdir,condname{c},n,s);
        eval(sprintf('save %s cc_ln_ln',file_name));
        file_name=sprintf('%s/%s_top%d_mps_mem_sub%02d',psdir,condname{c},n,s);
