...
ln_r=zeros(xlength,ylength,zlength,3);
        %perpare data
        load(sprintf('%s/encoding_sub%02d.mat',labeldir,sub));
        list_subln=subln;
        list_subln(:,Msub)=sub;
        list_subln(:,Mphase)=1;
        load(sprintf('%s/test_sub%02d.mat',labeldir,sub));
        list_submem=submem;
        list_submem(:,Msub)=sub;
        ...
        all_pID1=[]; all_pID2=[]; all_wID1=[]; all_wID2=[]; all_Rcate=[]; all_mem1=[]; all_mem2=[]; all_phase1=[]; all_phase2=[]; all_cate1=[];  all_cate2=[]; check_run=[]; check_set=[]; check_cate=[];all_run1=[];all_run2=[];all_set1=[];all_set2=[];
        for k=2:TN
