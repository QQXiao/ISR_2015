...
idx_mem2=find(all_phase1==2 & all_phase2==2 & all_run1==2 & all_pID1~=all_pID2 & check_run==1 & check_set==0 & all_mem1==1 & all_mem2==1);
        ...
        cc_ln=1-pdist(data_ln_all(:,:),'correlation');
        cc_mem=1-pdist(data_mem_all(:,:),'correlation');
            tcc_ln=squareform(cc_ln);
            tcc_mem=squareform(cc_mem);
    eval(sprintf('save %s/sub1_roi1_ln.txt tcc_ln -ascii -tabs', resultdir));
    eval(sprintf('save %s/sub1_roi1_mem.txt tcc_mem -ascii -tabs', resultdir));
    end
end
end
