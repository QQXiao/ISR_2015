...
subdata <- item_data
itemInfo_actmean <- lmer(RSFG_rsadiff~DG_ln_actmean+(1+DG_ln_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RSFG_rsadiff~1+(1+DG_ln_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RSFG_rsadiff~DG_ln_actmean+(1+DG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RSFG_rsadiff~1+(1+DG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RSFG_DG.txt",sep="\t")
