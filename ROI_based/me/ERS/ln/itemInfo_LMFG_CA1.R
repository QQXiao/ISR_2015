...
resultdir <- paste(basedir,"/me/tmap/results/ERS/ln",sep="/")
setwd(resultdir)
...
subdata <- item_data
itemInfo_actmean <- lmer(LMFG_rsadiff~CA1_ln_actmean+(1+CA1_ln_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LMFG_rsadiff~1+(1+CA1_ln_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LMFG_rsadiff~CA1_ln_actmean+(1+CA1_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LMFG_rsadiff~1+(1+CA1_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LMFG_CA1.txt",sep="\t")
