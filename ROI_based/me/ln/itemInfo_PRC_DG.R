...
subdata <- item_data
itemInfo_actmean <- lmer(PRC_rsadiff~DG_actmean+(1+DG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(PRC_rsadiff~1+(1+DG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(PRC_rsadiff~DG_actmean+(1+DG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(PRC_rsadiff~1+(1+DG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_PRC_DG.txt",sep="\t")
