...
subdata <- item_data
itemInfo_actmean <- lmer(p97_rsadiff~LIFG_actmean+(1+LIFG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(p97_rsadiff~1+(1+LIFG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(p97_rsadiff~LIFG_actmean+(1+LIFG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(p97_rsadiff~1+(1+LIFG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_p97_LIFG.txt",sep="\t")
