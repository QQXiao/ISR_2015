...
subdata <- item_data
itemInfo_actmean <- lmer(RVVC_rsadiff~RpPHG_actmean+(1+RpPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RVVC_rsadiff~1+(1+RpPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RVVC_rsadiff~RpPHG_actmean+(1+RpPHG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RVVC_rsadiff~1+(1+RpPHG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RVVC_RpPHG.txt",sep="\t")
