...
subdata <- item_data
itemInfo_actmean <- lmer(ERC_rsadiff~ERC_actmean+(1+ERC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(ERC_rsadiff~1+(1+ERC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(ERC_rsadiff~ERC_actmean+(1+ERC_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(ERC_rsadiff~1+(1+ERC_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_ERC_ERC.txt",sep="\t")
