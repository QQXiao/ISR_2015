...
subdata <- item_data
itemInfo_actmean <- lmer(LpPHG_rsadiff~RANG_actmean+(1+RANG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LpPHG_rsadiff~1+(1+RANG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LpPHG_rsadiff~RANG_actmean+(1+RANG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LpPHG_rsadiff~1+(1+RANG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LpPHG_RANG.txt",sep="\t")
