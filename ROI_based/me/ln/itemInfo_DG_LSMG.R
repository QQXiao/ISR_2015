...
subdata <- item_data
itemInfo_actmean <- lmer(DG_rsadiff~LSMG_actmean+(1+LSMG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(DG_rsadiff~1+(1+LSMG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(DG_rsadiff~LSMG_actmean+(1+LSMG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(DG_rsadiff~1+(1+LSMG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_DG_LSMG.txt",sep="\t")
