...
subdata <- item_data
itemInfo_actmean <- lmer(ERC_rsadiff~RANG_actmean+(1+RANG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(ERC_rsadiff~1+(1+RANG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(ERC_rsadiff~RANG_actmean+(1+RANG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(ERC_rsadiff~1+(1+RANG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_ERC_RANG.txt",sep="\t")
