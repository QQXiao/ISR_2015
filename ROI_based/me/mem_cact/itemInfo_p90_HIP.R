...
itemInfo_actmean <- lmer(p90_rsadiff~HIP_actmean+p90_actmean+(1+HIP_actmean+p90_actmean|subid)+(1+HIP_actmean+p90_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(p90_rsadiff~p90_actmean+(1+HIP_actmean+p90_actmean|subid)+(1+HIP_actmean+p90_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actrep <- lmer(p90_rsadiff~HIP_actrep+p90_actmean+(1+HIP_actrep+p90_actmean|subid)+(1+HIP_actrep+p90_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actrep.null <- lmer(p90_rsadiff~p90_actmean+(1+HIP_actrep+p90_actmean|subid)+(1+HIP_actrep+p90_actmean|pid),REML=FALSE,data=subdata)
itemInfo_rsadiff <- lmer(p90_rsadiff~HIP_rsadiff+p90_actmean+(1+HIP_rsadiff+p90_actmean|subid)+(1+HIP_rsadiff+p90_actmean|pid),REML=FALSE,data=subdata)
itemInfo_rsadiff.null <- lmer(p90_rsadiff~p90_actmean+(1+HIP_rsadiff+p90_actmean|subid)+(1+HIP_rsadiff+p90_actmean|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfo_p90_HIP.txt",sep="\t")
