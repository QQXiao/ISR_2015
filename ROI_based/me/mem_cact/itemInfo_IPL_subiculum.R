...
library(lme4)
#
#roi_name={'IPL','IPL','dLOC',...
#>---'subiculum','SMG','IFG','HIP',...
#>---'subiculum','CA2','DG','CA3','subiculum','ERC'};
# Read in your data as an R dataframe
basedir <- c("/seastor/helenhelen/ISR_2015")
resultdir <- paste(basedir,"/me/results/mem_cact",sep="/")
setwd(resultdir)
...
itemInfo_actmean <- lmer(IPL_rsadiff~subiculum_actmean+IPL_actmean+(1+subiculum_actmean+IPL_actmean|subid)+(1+subiculum_actmean+IPL_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(IPL_rsadiff~IPL_actmean+(1+subiculum_actmean+IPL_actmean|subid)+(1+subiculum_actmean+IPL_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actrep <- lmer(IPL_rsadiff~subiculum_actrep+IPL_actmean+(1+subiculum_actrep+IPL_actmean|subid)+(1+subiculum_actrep+IPL_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actrep.null <- lmer(IPL_rsadiff~IPL_actmean+(1+subiculum_actrep+IPL_actmean|subid)+(1+subiculum_actrep+IPL_actmean|pid),REML=FALSE,data=subdata)
itemInfo_rsadiff <- lmer(IPL_rsadiff~subiculum_rsadiff+IPL_actmean+(1+subiculum_rsadiff+IPL_actmean|subid)+(1+subiculum_rsadiff+IPL_actmean|pid),REML=FALSE,data=subdata)
itemInfo_rsadiff.null <- lmer(IPL_rsadiff~IPL_actmean+(1+subiculum_rsadiff+IPL_actmean|subid)+(1+subiculum_rsadiff+IPL_actmean|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfo_IPL_subiculum.txt",sep="\t")
