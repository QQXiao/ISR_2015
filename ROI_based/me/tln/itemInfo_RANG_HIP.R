...
subdata <- item_data
itemInfo_actmean <- lmer(RANG_rsadiff~HIP_actmean+(1+HIP_actmean|subid)+(1+HIP_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RANG_rsadiff~1+(1+HIP_actmean|subid)+(1+HIP_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RANG_rsadiff~HIP_actmean+(1+HIP_rsadiff|subid)+(1+HIP_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RANG_rsadiff~1+(1+HIP_rsadiff|subid)+(1+HIP_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RANG_HIP.txt",sep="\t")
