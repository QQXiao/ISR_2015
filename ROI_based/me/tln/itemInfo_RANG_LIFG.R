...
subdata <- item_data
itemInfo_actmean <- lmer(RANG_rsadiff~LIFG_actmean+(1+LIFG_actmean|subid)+(1+LIFG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RANG_rsadiff~1+(1+LIFG_actmean|subid)+(1+LIFG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RANG_rsadiff~LIFG_actmean+(1+LIFG_rsadiff|subid)+(1+LIFG_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RANG_rsadiff~1+(1+LIFG_rsadiff|subid)+(1+LIFG_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RANG_LIFG.txt",sep="\t")
