...
subdata <- item_data
itemInfo_actmean <- lmer(p97_rsadiff~HIP_actmean+(1+HIP_actmean|subid)+(1+HIP_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(p97_rsadiff~1+(1+HIP_actmean|subid)+(1+HIP_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(p97_rsadiff~HIP_actmean+(1+HIP_rsadiff|subid)+(1+HIP_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(p97_rsadiff~1+(1+HIP_rsadiff|subid)+(1+HIP_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_p97_HIP.txt",sep="\t")
