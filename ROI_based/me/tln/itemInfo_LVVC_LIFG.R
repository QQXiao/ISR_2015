...
subdata <- item_data
itemInfo_actmean <- lmer(LVVC_rsadiff~LIFG_actmean+(1+LIFG_actmean|subid)+(1+LIFG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LVVC_rsadiff~1+(1+LIFG_actmean|subid)+(1+LIFG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LVVC_rsadiff~LIFG_actmean+(1+LIFG_rsadiff|subid)+(1+LIFG_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LVVC_rsadiff~1+(1+LIFG_rsadiff|subid)+(1+LIFG_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LVVC_LIFG.txt",sep="\t")
