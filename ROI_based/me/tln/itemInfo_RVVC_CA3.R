...
subdata <- item_data
itemInfo_actmean <- lmer(RVVC_rsadiff~CA3_actmean+(1+CA3_actmean|subid)+(1+CA3_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RVVC_rsadiff~1+(1+CA3_actmean|subid)+(1+CA3_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RVVC_rsadiff~CA3_actmean+(1+CA3_rsadiff|subid)+(1+CA3_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RVVC_rsadiff~1+(1+CA3_rsadiff|subid)+(1+CA3_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RVVC_CA3.txt",sep="\t")
