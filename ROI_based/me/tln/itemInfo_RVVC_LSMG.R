...
subdata <- item_data
itemInfo_actmean <- lmer(RVVC_rsadiff~LSMG_actmean+(1+LSMG_actmean|subid)+(1+LSMG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RVVC_rsadiff~1+(1+LSMG_actmean|subid)+(1+LSMG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RVVC_rsadiff~LSMG_actmean+(1+LSMG_rsadiff|subid)+(1+LSMG_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RVVC_rsadiff~1+(1+LSMG_rsadiff|subid)+(1+LSMG_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RVVC_LSMG.txt",sep="\t")
