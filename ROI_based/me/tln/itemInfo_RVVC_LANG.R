...
subdata <- item_data
itemInfo_actmean <- lmer(RVVC_rsadiff~LANG_actmean+(1+LANG_actmean|subid)+(1+LANG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RVVC_rsadiff~1+(1+LANG_actmean|subid)+(1+LANG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RVVC_rsadiff~LANG_actmean+(1+LANG_rsadiff|subid)+(1+LANG_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RVVC_rsadiff~1+(1+LANG_rsadiff|subid)+(1+LANG_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RVVC_LANG.txt",sep="\t")
