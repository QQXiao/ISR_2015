...
subdata <- item_data
itemInfo_actmean <- lmer(LSMG_rsadiff~LIFG_actmean+(1+LIFG_actmean|subid)+(1+LIFG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LSMG_rsadiff~1+(1+LIFG_actmean|subid)+(1+LIFG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LSMG_rsadiff~LIFG_actmean+(1+LIFG_rsadiff|subid)+(1+LIFG_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LSMG_rsadiff~1+(1+LIFG_rsadiff|subid)+(1+LIFG_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LSMG_LIFG.txt",sep="\t")
