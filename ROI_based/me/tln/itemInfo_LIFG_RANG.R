...
subdata <- item_data
itemInfo_actmean <- lmer(LIFG_rsadiff~RANG_actmean+(1+RANG_actmean|subid)+(1+RANG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LIFG_rsadiff~1+(1+RANG_actmean|subid)+(1+RANG_actmean|pid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LIFG_rsadiff~RANG_actmean+(1+RANG_rsadiff|subid)+(1+RANG_rsadiff|pid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LIFG_rsadiff~1+(1+RANG_rsadiff|subid)+(1+RANG_rsadiff|pid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LIFG_RANG.txt",sep="\t")
