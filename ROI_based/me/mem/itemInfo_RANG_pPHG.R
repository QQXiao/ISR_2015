...
subdata <- item_data
itemInfo_actmean <- lmer(RANG_rsadiff~pPHG_actmean+(1+pPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RANG_rsadiff~1+(1+pPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RANG_rsadiff~pPHG_actmean+(1+pPHG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RANG_rsadiff~1+(1+pPHG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RANG_pPHG.txt",sep="\t")
