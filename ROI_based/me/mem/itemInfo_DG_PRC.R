...
subdata <- item_data
itemInfo_actmean <- lmer(DG_rsadiff~PRC_actmean+(1+PRC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(DG_rsadiff~1+(1+PRC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(DG_rsadiff~PRC_actmean+(1+PRC_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(DG_rsadiff~1+(1+PRC_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_DG_PRC.txt",sep="\t")
