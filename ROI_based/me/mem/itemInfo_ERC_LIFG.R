...
subdata <- item_data
itemInfo_actmean <- lmer(ERC_rsadiff~LIFG_actmean+(1+LIFG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(ERC_rsadiff~1+(1+LIFG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(ERC_rsadiff~LIFG_actmean+(1+LIFG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(ERC_rsadiff~1+(1+LIFG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_ERC_LIFG.txt",sep="\t")
