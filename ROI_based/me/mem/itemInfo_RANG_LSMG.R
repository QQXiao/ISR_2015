...
subdata <- item_data
itemInfo_actmean <- lmer(RANG_rsadiff~LSMG_actmean+(1+LSMG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RANG_rsadiff~1+(1+LSMG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RANG_rsadiff~LSMG_actmean+(1+LSMG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RANG_rsadiff~1+(1+LSMG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RANG_LSMG.txt",sep="\t")
