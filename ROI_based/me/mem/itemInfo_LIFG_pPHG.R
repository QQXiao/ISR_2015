...
subdata <- item_data
itemInfo_actmean <- lmer(LIFG_rsadiff~pPHG_actmean+(1+pPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LIFG_rsadiff~1+(1+pPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LIFG_rsadiff~pPHG_actmean+(1+pPHG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LIFG_rsadiff~1+(1+pPHG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LIFG_pPHG.txt",sep="\t")
