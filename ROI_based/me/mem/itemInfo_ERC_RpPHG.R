...
subdata <- item_data
itemInfo_actmean <- lmer(ERC_rsadiff~RpPHG_actmean+(1+RpPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(ERC_rsadiff~1+(1+RpPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(ERC_rsadiff~RpPHG_actmean+(1+RpPHG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(ERC_rsadiff~1+(1+RpPHG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_ERC_RpPHG.txt",sep="\t")
