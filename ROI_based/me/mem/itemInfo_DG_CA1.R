...
subdata <- item_data
itemInfo_actmean <- lmer(DG_rsadiff~CA1_actmean+(1+CA1_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(DG_rsadiff~1+(1+CA1_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(DG_rsadiff~CA1_actmean+(1+CA1_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(DG_rsadiff~1+(1+CA1_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_DG_CA1.txt",sep="\t")
