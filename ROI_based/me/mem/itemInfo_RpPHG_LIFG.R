...
subdata <- item_data
itemInfo_actmean <- lmer(RpPHG_rsadiff~LIFG_actmean+(1+LIFG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(RpPHG_rsadiff~1+(1+LIFG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(RpPHG_rsadiff~LIFG_actmean+(1+LIFG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(RpPHG_rsadiff~1+(1+LIFG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_RpPHG_LIFG.txt",sep="\t")
