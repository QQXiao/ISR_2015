...
subdata <- item_data
itemInfo_actmean <- lmer(pPHG_rsadiff~ERC_actmean+(1+ERC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(pPHG_rsadiff~1+(1+ERC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(pPHG_rsadiff~ERC_actmean+(1+ERC_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(pPHG_rsadiff~1+(1+ERC_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_pPHG_ERC.txt",sep="\t")
