...
subdata <- item_data
itemInfo_actmean <- lmer(DG_rsadiff~ERC_actmean+(1+ERC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(DG_rsadiff~1+(1+ERC_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(DG_rsadiff~ERC_actmean+(1+ERC_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(DG_rsadiff~1+(1+ERC_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_DG_ERC.txt",sep="\t")
