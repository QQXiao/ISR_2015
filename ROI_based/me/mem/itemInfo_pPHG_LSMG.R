...
subdata <- item_data
itemInfo_actmean <- lmer(pPHG_rsadiff~LSMG_actmean+(1+LSMG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(pPHG_rsadiff~1+(1+LSMG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(pPHG_rsadiff~LSMG_actmean+(1+LSMG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(pPHG_rsadiff~1+(1+LSMG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_pPHG_LSMG.txt",sep="\t")
