...
subdata <- item_data
itemInfo_actmean <- lmer(subiculum_rsadiff~DG_actmean+(1+DG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(subiculum_rsadiff~1+(1+DG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(subiculum_rsadiff~DG_actmean+(1+DG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(subiculum_rsadiff~1+(1+DG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_subiculum_DG.txt",sep="\t")
