...
subdata <- item_data
itemInfo_actmean <- lmer(LVVC_rsadiff~pPHG_actmean+(1+pPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_actmean.null <- lmer(LVVC_rsadiff~1+(1+pPHG_actmean|subid),REML=FALSE,data=subdata)
itemInfo_di <- lmer(LVVC_rsadiff~pPHG_actmean+(1+pPHG_rsadiff|subid),REML=FALSE,data=subdata)
itemInfo_di.null <- lmer(LVVC_rsadiff~1+(1+pPHG_rsadiff|subid),REML=FALSE,data=subdata)
...
write.matrix(r.itemInfo,file="itemInfso_LVVC_pPHG.txt",sep="\t")
