...
resultdir1=sprintf('%s/ROI_based/subs_within_between/Test4/method1',basedir);
resultdir2=sprintf('%s/ROI_based/subs_within_between/Test4/method2',basedir);
...
for np=1:1000 %permutation for 1000 times for baseline
    ...
    for c=1:length(Cond_Name)
        eval(sprintf('c%s1(:,:,np)=mean(all_c%s1,3);',Cond_Name{c},Cond_Name{c}));
        eval(sprintf('c%s2(:,:,np)=mean(all_c%s2,3);',Cond_Name{c},Cond_Name{c}));
        eval(sprintf('fps_%s(:,:,np)=mean(all_ps_%s,3);',Cond_Name{c},Cond_Name{c}));
    end
    %clear no use matrix
    ...
    mean_rs_ln1_matrix=squeeze(mean(all_rs_ln1_matrix,2));
    ...
    bs_ln_m1,bs_mem_m1,bs_ERS_m1,...
    ws_ln_m2,ws_mem_m2,ws_ERS_m2,bs_ln_m2,bs_mem_m2,bs_ERS_m2] = ...
    Cal_RS_similarity(mean_rs_ln1_matrix,mean_rs_ln2_matrix,mean_rs_mem1_matrix,mean_rs_mem2_matrix,subs,Cond_Name,all_sub1,all_sub2,check_set);
    %get within sub's or cross subs' correlation
    for c=1:length(Cond_Name)
        eval(sprintf('c%s1(:,2,np) = bs_%s_m1;',Cond_Name{c},Cond_Name{c}));
        eval(sprintf('c%s1(:,3,np) = ws_%s_m1;',Cond_Name{c},Cond_Name{c}));
        eval(sprintf('c%s2(:,2,np) = bs_%s_m2;',Cond_Name{c},Cond_Name{c}));
        eval(sprintf('c%s2(:,3,np) = ws_%s_m2;',Cond_Name{c},Cond_Name{c}));
        for sf=subs
            for sff=subs
                if sff~=sf
                    eval(sprintf('fps_%s(sf,sff,np) = ps_%s(sf,sff);',Cond_Name{c},Cond_Name{c}));
                end
            end
            %rank
            eval(sprintf('Nrank_%s(sf,np)=sum(fps_%s(sf,sf)>fps_%s(sf,setdiff(subs,[2 sf])));',Cond_Name{c},Cond_Name{c},Cond_Name{c}));
        end
    end %end cond
end %end permutation for baseline
%get z
for c=1:length(Cond_Name)
    ...
    eval(sprintf('save %s/b_%s_%s.txt %s_z1 -ascii -tabs', resultdir1,Cond_Name{c},roi_name{roi},Cond_Name{c}));
    eval(sprintf('save %s/b_rank_%s_%s.txt Nrank_%s -ascii -tabs', resultdir1,Cond_Name{c},roi_name{roi},Cond_Name{c}));
    eval(sprintf('save %s/b_%s_%s.txt %s_z2 -ascii -tabs', resultdir2,Cond_Name{c},roi_name{roi},Cond_Name{c}));
end
eval(sprintf('save %s/b_ps_%s.mat fps_ln fps_mem fps_ERS', resultdir1,roi_name{roi}));
end %function
