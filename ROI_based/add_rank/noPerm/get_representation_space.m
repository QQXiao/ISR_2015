...
resultdir=sprintf('%s/ROI_based/subs_within_between/noPerm',basedir);
...
roi=r;
    ...
    for sff=1:length(subs);
        ...
        %% data
        for ssi=1:length(subs);
            si=subs(ssi);
            sdata(sff,ssi)=cc_ln(all_sub1==sf & all_sub2==si & check_ln==0);
        end
    end %end subs
    ln=cln;mem=cmem;ERS12=cERS12;ERS21=cERS21;
    sdata_z=0.5*(log(1+sdata)-log(1-sdata));
    ...
    eval(sprintf('save %s/csubs_%s.txt sdata_z -ascii -tabs', resultdir,roi_name{roi}));
