...
for np=1:1000
    ...
    all_ln1_data{np}=ln1_matrix;
    all_ln2_data{np}=ln2_matrix;
    all_mem1_data{np}=mem1_matrix;
    all_mem2_data{np}=mem2_matrix;
    ...
    eval(sprintf('save %s/BL_all_ln1_%s.mat all_ln1_data', resultdir,roi_name{roi}));
    eval(sprintf('save %s/BL_all_ln2_%s.mat all_ln2_data', resultdir,roi_name{roi}));
    eval(sprintf('save %s/BL_all_mem1_%s.mat all_mem1_data', resultdir,roi_name{roi}));
    eval(sprintf('save %s/BL_all_mem2_%s.mat all_mem2_data', resultdir,roi_name{roi}));
end %function
