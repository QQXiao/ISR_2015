...
for np=1:1000 %permutation for 1000 times
    ...
    eval(sprintf('save %s/all_ln1_%s.mat all_ln1_data', resultdir,roi_name{roi}));
    eval(sprintf('save %s/all_ln2_%s.mat all_ln2_data', resultdir,roi_name{roi}));
    eval(sprintf('save %s/all_mem1_%s.mat all_mem1_data', resultdir,roi_name{roi}));
    eval(sprintf('save %s/all_mem2_%s.mat all_mem2_data', resultdir,roi_name{roi}));
end %function
