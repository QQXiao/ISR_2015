...
for r=1:2
    for g=1:2
        ...
        all_data1=all_data.img(:,:,:,fix((onset+(2*t)+4)/2));
        filename=sprintf('%s/sub%02d_encoding_run%d_set%d_%dTR.nii',resultdir,sub,r,g,t);
        ...
        all_data.img=zscore(all_data.img,0,4); % normalize along the time dimension
        all_data1=all_data.img(:,:,:,fix((onset+(2*t)+4)/2));
        filename=sprintf('%s/sub%02d_test_run%d_set%d_%dTR.nii',resultdir,sub,r,g,t);
        all_data.img=all_data1;
        all_data.hdr.dime.dim(5)=TN; % dimension chagne to
        save_untouch_nii(all_data, filename);
        system(sprintf('gzip -f %s',filename));
